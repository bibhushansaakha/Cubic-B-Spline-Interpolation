...
def interpolate_image(image, scale_factor):
    ...
    # Apply slight Gaussian blur to the interpolated image
    blurred_values = interpolated_values.astype(np.float32)
    blurred_values = (blurred_values + np.roll(blurred_values, 1, axis=0) + np.roll(blurred_values, -1, axis=0) +
                      np.roll(blurred_values, 1, axis=1) + np.roll(blurred_values, -1, axis=1)) / 5
...
def main():
    ...
    # Perform interpolation and apply slight blur
