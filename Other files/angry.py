...
def generate_control_points(image_size, num_points):
        ...
        for x in range(width):
            ...
            for c in range(3):  # Iterate over RGB channels
                ...
                for i in range(len(control_points)):
                    weighted_sum += basis_functions_x[i] * basis_functions_y[i] * pixel_values[int(control_points[i][1] * (height - 1)), int(control_points[i][0] * (width - 1)), c]
...
def main():
    image_url = "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9b/Flag_of_Nepal.svg/1200px-Flag_of_Nepal.svg.png"  # Replace with the actual image URL
    ...
    control_points = generate_control_points(image.size, num_control_points)
